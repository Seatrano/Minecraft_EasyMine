...
function NameManager.generateNewName()
    ...
    -- WICHTIG: Namen generieren UND sofort reservieren
    -- Dies muss atomar geschehen (innerhalb des assignmentLock!)
...
function ConnectionHandler.handleNewConnection(id, message, now)
    ...
    -- SOFORT in masterConfig schreiben (noch im Lock!)
    if not masterConfig.turtles[turtleName] then
        masterConfig.turtles[turtleName] = {
            turtleName = turtleName
        }
    else
        -- Alten Chunk freigeben falls vorhanden
...
function MonitorDisplay.showTurtle(turtle, row)
    ...
    State.mon.write(
        name .. " X:" .. x .. " Z:" .. z .. " Y:" .. y .. " Dir:" .. dirStr .. " Fuel:" .. fuel .. " Chunk:" .. chunk ..
            " Status:" .. status)
